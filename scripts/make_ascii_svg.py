...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "sreenikesh-ascii.svg")
...
# ---- crop away large white margins around the subject --------------------
# The prep step places the subject on white. Find the non-white region
# and crop around it so the face/upper body occupies more of the ASCII frame.
bbox = im.point(lambda p: 255 if p < 245 else 0).getbbox()
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">sreenikesh@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'sreenikesh@github:~$ whoami <tspan fill="{INK}">Sreenikesh A</tspan></text>')
